/**
 * Accepted solution to Genefinderx9000
 * Aho-Corasick algorithm with exit link optimization and array indexing:
 * O(N+M+Z)
 */
#include <bits/stdc++.h>
#include <functional>
...
private:
  ...
  int M,                // Number of nodes in trie
      ...
      _exit[MAX_M],     // Exit links   : memoization of exit link
      out[MAX_M];       // out[m]       : contains index of all queries that ends at node 'm'

  int exit(int m) {
    static auto isLeaf = [this](int m) { return out[m] > root(); };
    if (_exit[m] == FAIL) {
      int f = fail[m];
      if (!isLeaf(f) && f != root()) f = exit(f);
      if (f == FAIL) f = root();
      _exit[m] = f;
    }
    return _exit[m];
...
public:
  ...
  vvi find(MultiString<MAX_N>&s) {
    ...
    for (int n = 0, m = root(); n < N; ++n) {
      ...
      // Follow end links : O(Z)
      for (int f = m;
           f != root();
           f = exit(f))
      {
        if (out[f] > root()) {
          ans[out[f] - 1].push_back(s.strnum(n - queries[out[f] - 1].size() + 1));
        }
      } 
    }
    return ans;
...
int main()
{
  ...
  vvi matches = matcher.find(ms);
  auto joiner = os_joiner(" ");
  for (int k = 0; k < K; ++k) {
    // Filter out duplicates in O(dist(max(z),min(z))) where Z is total count of matches
    auto ans_end = unique(matches[k].beg(), matches[k].end()); 
    cout << (int)(ans_end - matches[k].beg());
    for (auto it = matches[k].beg(); it < ans_end; ++it)
      if (it != ans_end){
        cout << " " << *it;
      }
    cout << "\n";
