...
public:
  ...
  vvi find(MultiString<MAX_N>&s)
  {
    ...
    for (int n = 0, m = root(); n < N; ++n) {
      ...
      // if (m % 10000 == 0) cout << "Finding " << s[n] << " from m#" << m << endl;
      m = findNode(m, s[n]);
...
// Input should likely be 50 million in total, so each is 25 mil
...
int main()
{
  ...
  vvi matches = matcher.find(ms);   // O(N+M+Z)
  for (int k = 0; k < K; ++k) {     // O(N×logZ)
    set<int> ans(ALL(matches[k])); 
    cout << ans.size();
    if (!ans.empty()) {
      cout << " ";
      copy(ALL(ans), joiner);
